...
import { getCitas, crearCita, getPacientes, cancelarCita } from "../../services/api"
...
const PACIENTES_HOY_MOCK: { id: number; nombre: string; hora: string; tipo: string; estado: "confirmada" | "pendiente" }[] = []
...
export default function Dashboard() {
  ...
  // Citas formateadas para FullCalendar
  ...
  async function cargarDatos() {
    try {
      ...
      if (resCitas.success) {
        ...
        const eventos = (resCitas.data as any[]).map((cita) => ({
          id: String(cita.id),
  ...
  async function handleGuardarCita(datos: DatosCita) {
    try {
      ...
      const respuesta = await crearCita({
        ...
        profesionalId: 0, // el backend lo toma del token
        fecha: datos.fecha,
        hora: datos.hora,
        estado: "pendiente",
      })
      ...
      if (respuesta.success) {
        ...
        await cargarDatos() // recarga el calendario con la cita nueva
      } else {
          ...
          <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
              ...
              <p className="text-3xl font-bold text-dark">
                {eventosCitas.filter(e => e.backgroundColor !== "#94a3b8").length}
              </p>
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
              ...
              <p className="text-3xl font-bold text-dark">
                {eventosCitas.filter(e => e.start.startsWith(new Date().toISOString().split('T')[0])).length}
              </p>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 flex-1">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="hidden sm:flex items-center gap-4 text-xs text-slate-400">
                ...
                <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-slate-300 inline-block"></span>Cancelada</span>
              </div>
            </div>
            ...
            {PACIENTES_HOY_MOCK.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                {PACIENTES_HOY_MOCK.map((paciente) => (
                  <div key={paciente.id} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl hover:bg-background transition-colors cursor-pointer">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${paciente.estado === "confirmada" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}>
