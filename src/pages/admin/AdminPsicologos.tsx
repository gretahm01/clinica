...
import { getTodosPsicologos, crearPsicologo } from "../../services/api"
...
export default function AdminPsicologos() {
  ...
  const [form, setForm] = useState({ nombre: "", apellido: "", email: "", telefono: "", cedula: "", fechaNacimiento: "" })
  ...
  async function cargarLista() {
    ...
    try {
      const res = await getTodosPsicologos()
      if (res.success) setPsicologos(res.data)
      else setError("No se pudieron cargar los psicólogos")
    } catch {
  ...
  async function handleGuardar(e: React.FormEvent) {
    ...
    setGuardando(true)
    try {
      // El backend se encarga de asignar 'password123'
      const res = await crearPsicologo(form as any)
      if (res.success) {
        setModalAbierto(false)
        setForm({ nombre: "", apellido: "", email: "", telefono: "", cedula: "", fechaNacimiento: "" })
        await cargarLista()
      } else {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-dark">Psicólogos</h1>
              <p className="text-slate-400 text-sm mt-0.5">Gestión de personal clínico</p>
            </div>
          ...
          {cargando ? (
            ...
            <div className="text-center py-20 bg-white border border-slate-100 rounded-2xl shadow-sm">
              <p className="text-slate-400 font-medium">No hay psicólogos registrados.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {filtrados.map(p => (
                <div key={p.id} className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl bg-blue-100 text-blue-600 flex items-center justify-center font-bold text-lg uppercase flex-shrink-0">
                    {p.nombre?.[0] || ""}{p.apellido?.[0] || ""}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-dark truncate capitalize">{p.nombre} {p.apellido}</p>
                    <p className="text-xs text-slate-400 truncate">{p.email}</p>
                    <p className="text-[10px] bg-slate-50 text-slate-500 px-2 py-0.5 rounded uppercase font-bold w-fit mt-1 border border-slate-100">
                      Cédula: {p.cedula || "N/A"}
                    </p>
                  </div>
                  <button onClick={() => navigate(`/admin/psicologos/${p.id}`)} className="text-xs text-primary font-bold hover:underline">Ver Perfil</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-xl font-bold text-dark">Alta de Psicólogo</h2>
            ...
            <form onSubmit={handleGuardar} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="block text-[11px] font-bold text-slate-500 uppercase mb-1">Cédula Prof.</label><input required type="text" value={form.cedula} onChange={e=>setForm({...form, cedula: e.target.value})} className="w-full border border-slate-200 rounded-xl p-2.5 text-sm" /></div>
              </div>
              <div><label className="block text-[11px] font-bold text-slate-500 uppercase mb-1">Fecha Nacimiento</label><input required type="date" value={form.fechaNacimiento} onChange={e=>setForm({...form, fechaNacimiento: e.target.value})} className="w-full border border-slate-200 rounded-xl p-2.5 text-sm text-slate-600" /></div>
              
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 text-center mt-2">
                <p className="text-[10px] text-blue-600 font-bold uppercase tracking-wider">Contraseña de acceso</p>
              ...
              <div className="flex gap-3 pt-3">
                ...
                <button type="submit" disabled={guardando} className="flex-1 py-2.5 rounded-xl font-bold text-white bg-primary hover:bg-primary-hover transition-colors disabled:opacity-50">{guardando ? 'Guardando...' : 'Registrar'}</button>
              </div>
            </form>
          </div>
        </div>
