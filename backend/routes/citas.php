...
if ($method === 'GET' && isset($partes[0]) && $partes[0] === 'paciente' && isset($partes[1]) && is_numeric($partes[1])) {
    ...
    $sql = "
        SELECT 
            ...
            a.patient_id        AS pacienteId,
            a.professional_id   AS profesionalId,
...
// ===========================
// GET /citas — todas las citas del profesional
// ===========================
if ($method === 'GET') {
...
if ($method === 'POST') {
    ...
    ");
    $stmtCheck->bind_param("iss", $professional_id, $fecha, $hora);
    $stmtCheck->execute();
    if ($stmtCheck->get_result()->num_rows > 0) {
        http_response_code(409);
        echo json_encode(["success" => false, "message" => "Ya tienes una cita agendada a esa hora"]);
        exit();
    }
    $stmtCheck->close();
    ...
    $status = "pendiente";
    $stmt = $conn->prepare("
        INSERT INTO appointment (patient_id, professional_id, appointment_date, appointment_time, status)
        VALUES (?, ?, ?, ?, ?)
    ");
    $stmt->bind_param("iisss", $pacienteId, $professional_id, $fecha, $hora, $status);
    $stmt->execute();
